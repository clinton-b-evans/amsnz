...
def commodity_list_view(request):
    commodities = Commodity.objects.exclude(investment=0)
    ...
    for commodity in commodities:
        ...
        transactions_table.append({
            ...
            "profit_loss_percentage": ((float(currentMarketValue) - float(commodity.investment)) / float(commodity.investment)) * 100,
            "status": status
        })
